...
[[nodiscard]] constexpr inline unsigned char hex2char(TChar const ch) noexcept {
  ...
  if (ch >= static_cast<TChar>('a') && ch <= static_cast<TChar>('f')) return static_cast<unsigned char>(10 + ch - static_cast<TChar>('a'));
  if (ch >= static_cast<TChar>('A') && ch <= static_cast<TChar>('F')) return static_cast<unsigned char>(10 + ch - static_cast<TChar>('A'));
  return 0;
}
...
[[nodiscard]] constexpr inline bool is_hex(TChar const ch) noexcept {
  return (ch >= static_cast<TChar>('0') && ch <= static_cast<TChar>('9')) || (ch >= static_cast<TChar>('a') && ch <= static_cast<TChar>('f'))
...
class sha1 {
 public:
  ...
  [[nodiscard]] inline static uint32_t left_rotate(uint32_t value, size_t const count) noexcept { return (value << count) ^ (value >> (32 - count)); }
  ...
  uint32_t const* get_digest(digest32_t digest) {
    ...
    if (this->m_blockByteIndex > 56) {
      while (m_blockByteIndex != 0) { process_byte(0); }
      while (m_blockByteIndex < 56) { process_byte(0); }
    } else {
      while (m_blockByteIndex < 56) { process_byte(0); }
    }
    process_byte(0);
 ...
 private:
  void process_block() {
    ...
    for (size_t i = 16; i < 80; i++) { w[i] = left_rotate((w[i - 3] ^ w[i - 8] ^ w[i - 14] ^ w[i - 16]), 1); }
...
class uuid {
 public:
  ...
  [[nodiscard]] constexpr uuid_variant variant() const noexcept {
    if ((data[8] & 0x80) == 0x00) return uuid_variant::ncs;
  ...
  [[nodiscard]] constexpr uuid_version version() const noexcept {
    if ((data[6] & 0xF0) == 0x10) return uuid_version::time_based;
  ...
  [[nodiscard]] constexpr static bool is_valid_uuid(StringType const& in_str) noexcept {
    ...
    for (size_t i = hasBraces; i < str.size() - hasBraces; ++i) {
      ...
      if (index >= 16 || !detail::is_hex(str[i])) { return false; }
  ...
  [[nodiscard]] constexpr static std::optional<uuid> from_string(StringType const& in_str) noexcept {
    ...
    for (size_t i = hasBraces; i < str.size() - hasBraces; ++i) {
      ...
      if (index >= 16 || !detail::is_hex(str[i])) { return {}; }
...
class uuid_system_generator {
 public:
  ...
  uuid operator()() {
    ...
    if (FAILED(hr)) { throw std::system_error(hr, std::system_category(), "CoCreateGuid failed"); }

    std::array<uint8_t, 16> bytes = {{static_cast<unsigned char>((newId.Data1 >> 24) & 0xFF), static_cast<unsigned char>((newId.Data1 >> 16) & 0xFF),
    ...
    std::array<uint8_t, 16> arrbytes = {{bytes.byte0, bytes.byte1, bytes.byte2, bytes.byte3, bytes.byte4, bytes.byte5, bytes.byte6, bytes.byte7,
...
class uuid_time_generator {
  ...
  [[nodiscard]] bool get_mac_address() {
    if (device_address.has_value()) { return true; }
...
#else
    uint64_t l = static_cast<uint64_t>(uuid.data[0]) << 56 | static_cast<uint64_t>(uuid.data[1]) << 48 | static_cast<uint64_t>(uuid.data[2]) << 40
        | static_cast<uint64_t>(uuid.data[3]) << 32 | static_cast<uint64_t>(uuid.data[4]) << 24 | static_cast<uint64_t>(uuid.data[5]) << 16
        | static_cast<uint64_t>(uuid.data[6]) << 8 | static_cast<uint64_t>(uuid.data[7]);
    uint64_t h = static_cast<uint64_t>(uuid.data[8]) << 56 | static_cast<uint64_t>(uuid.data[9]) << 48 | static_cast<uint64_t>(uuid.data[10]) << 40
        | static_cast<uint64_t>(uuid.data[11]) << 32 | static_cast<uint64_t>(uuid.data[12]) << 24 | static_cast<uint64_t>(uuid.data[13]) << 16
